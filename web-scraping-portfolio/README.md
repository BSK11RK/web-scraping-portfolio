...
### 13. Google News（公式RSS × ニュース自動収集・翻訳・多形式出力）
- Google News 公式 RSS を利用して指定キーワード（例：AI）の最新ニュースを自動収集
- 記事タイトル・公開日時・リンクを整理して取得
- 英語タイトルを日本語に自動翻訳（deep_translator 使用）
- CSV / JSON / Excel の3形式で保存
- User-Agent を指定し、安定したデータ取得を実装
- ニュース収集・情報調査の自動化ツールとして活用可能

### 14. Google News（複数キーワード対応・統計付きニュース自動収集）
- Google News 公式 RSS を利用して複数キーワード（例：AI / Python / DX）の最新ニュースを自動収集
- 各記事のタイトル・公開日時・リンクを整理して取得
- 英語タイトルを日本語に自動翻訳（deep_translator 使用）
- キーワードごとにニュースを分類して保存
- タイトル文字数を自動計算し、以下の統計情報を表示
  - 総記事数
  - 平均タイトル文字数
  - 最大タイトル文字数
- CSV / JSON / Excel の3形式で保存
- User-Agent を指定し、安定したデータ取得を実装
- 技術トレンド調査・ニュースレポート自動生成に活用可能

### 15. 国会会議録 API（発言データ自動収集・多形式出力）
- 国立国会図書館が提供する「国会会議録検索 API」を利用して発言データを自動取得
- 指定キーワード（例：AI）を含む発言を検索
- 会議名・発言者・発言内容・発言日・発言URLを整理して取得
- API 経由のため HTML スクレイピング不要で安定したデータ取得が可能
- 発言データを pandas に格納し、以下の統計情報を自動算出
  - 総発言数
  - 発言者のユニーク数
- CSV / JSON / Excel の3形式で保存
- 政策分析・社会動向調査・テキスト分析用データ収集のベースとして活用可能

### 16. 世界都市 天気データ自動収集・可視化（Open-Meteo API）
- Open-Meteo の公式APIを利用して世界10都市の気象データを自動取得
- 現在気温（リアルタイム）と直近7日間の平均気温を同時に取得
- 東京・大阪・札幌・ロンドン・パリ・ベルリン・ニューヨーク・ロサンゼルス・ソウル・シンガポールに対応
- APIエラーを考慮した安全なデータ取得処理を実装
- CSV / JSON / Excel の3形式で保存
- matplotlib による都市別気温比較グラフを自動生成
- スクレイピングではなく API 利用のため、無料・安全・規約遵守で運用可能
- 気象データ分析・レポート自動生成案件への応用が可能
